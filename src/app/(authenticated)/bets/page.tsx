...
export default function BetsPage() {
  ...
  const handleGradeResults = useCallback(async () => {
    ...
    try {
      ...
      if (summary.totalGraded > 0) {
        setToast({ message: `${summary.totalGraded} bet${summary.totalGraded > 1 ? 's' : ''} graded automatically.`, type: 'success' });
  ...
  const filtered = useMemo(() => {
    ...
    return tabFiltered.filter(
      (b) => b.fight_name.toLowerCase().includes(s) || b.event_name.toLowerCase().includes(s)
    );
  }, [tabFiltered, search]);

  useEffect(() => { setCurrentPage(1); }, [currentTab, search]);
  ...
  const futureTotal = useMemo(() => {
    ...
    const totalProfit = tabFiltered.reduce((s, b) => s + b.stake_usd * (b.odds - 1), 0);
      ...
      <div className="page-header" style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
        ...
        <div style={{ display: 'flex', gap: 10 }}>
          ...
          <button className="btn-save" onClick={() => setShowModal(true)}>
            <i className="fa-solid fa-plus" style={{ marginRight: 6 }}></i> New Bet
          </button>
        </div>
      </div>
      ...
      <section className="fights-section glass-panel">
        <div className="panel-header">
          ...
          <div className="table-filters">
            <input type="text" placeholder="Search fighter or event..." className="search-input"
        ...
        {futureTotal && (
          <div className="future-summary-bar">
            <div className="future-summary-item">
              <span className="future-summary-label"><i className="fa-solid fa-coins"></i> Total Staked</span>
              <span className="future-summary-value">{fmt(futureTotal.totalStake)}</span>
            </div>
            <div className="future-summary-item">
              <span className="future-summary-label"><i className="fa-solid fa-arrow-trend-up"></i> Potential Profit</span>
              <span className="future-summary-value text-gold">+{fmt(futureTotal.totalProfit)}</span>
            </div>
            <div className="future-summary-item">
              <span className="future-summary-label"><i className="fa-solid fa-list-check"></i> Bets</span>
        ...
        {loading ? (
          ...
          <div style={{ padding: '60px 20px', textAlign: 'center' }}>
            ...
            <p style={{ color: 'var(--text-muted)', fontSize: 14, marginBottom: 16 }}>No bets recorded yet.</p>
            <button className="btn-save" onClick={() => setShowModal(true)}>
              <i className="fa-solid fa-plus" style={{ marginRight: 6 }}></i> Create First Bet
            </button>
          </div>
        ) : (
          <>
              ...
              return (
                <div key={b.id || i}
                  ...
                  <div className="bmc-stats">
                    <div className="bmc-stat"><span className="bmc-stat-label">Odds</span><span className="bmc-stat-value">{b.odds > 0 ? b.odds.toFixed(3) : '--'}</span></div>
                    <div className="bmc-stat"><span className="bmc-stat-label">Stake</span><span className="bmc-stat-value">{fmt(b.stake_usd)}</span></div>
                    {isFinishedTab && (<div className="bmc-stat"><span className="bmc-stat-label">P/L</span><span className={`bmc-stat-value ${plClass}`}>{plText}</span></div>)}
                    <div className="bmc-stat"><span className="bmc-stat-label">Date</span><span className="bmc-stat-value" style={{ fontSize: 12 }}>{b.date || '--'}</span></div>
                  </div>
          ...
          <div className="bets-desktop-table table-responsive">
            <table>
              <thead>
                <tr>
                  <th>Date</th><th>Event</th><th>Fight</th><th>Odds</th><th>Stake</th><th>Result</th>
                  {isFinishedTab && (<><th>P/L</th><th>Bankroll</th></>)}
                  <th style={{ width: 80, textAlign: 'center' }}>Actions</th>
                </tr>
              </thead>
              <tbody>
                {filtered.length === 0 ? (
                  <tr><td colSpan={isFinishedTab ? 9 : 7} style={{ textAlign: 'center', color: 'var(--text-muted)' }}>No bets found for this tab.</td></tr>
                ) : (
                  paginatedBets.map((b, i) => {
                    let plClass = '';
                    let plText = b.pl_usd ? fmt(b.pl_usd) : '--';
                      ...
                      <tr key={b.id || i}
                        ...
                        {isFinishedTab && (
                          <>
                            ...
                            <td>{b.id && bankrollMap.has(b.id) ? fmt(bankrollMap.get(b.id)!) : '--'}</td>
                          </>
                        )}
          ...
          <div className="pagination-bar">
            ...
            <div className="pagination-controls">
              ...
              {Array.from({ length: Math.min(5, totalPages) }, (_, idx) => {
                ...
                return (
                  <button key={page} className={`pagination-btn pagination-num ${currentPage === page ? 'active' : ''}`}
...
/** Original ResultBadge — portal dropdown, skips hidden badges */
function ResultBadge({
  bet, isEditing, onToggle, onSelect,
}: {
  ...
  useEffect(() => {
    if (isEditing && badgeRef.current) {
      // ★ Skip if badge is inside a display:none container (prevents duplicate)
  ...
  const isAutoGraded = !!(bet as Bet).graded_at;
  ...
  const dropdown = isEditing && pos
    ? createPortal(
        <div ref={dropdownRef} style={{
          ...
          background: 'rgb(15, 15, 18)', border: '1px solid rgba(255,255,255,0.12)',
          borderRadius: 10, padding: 6, zIndex: 9999, minWidth: 150,
          maxWidth: 'calc(100vw - 32px)',
          boxShadow: '0 8px 32px rgba(0,0,0,0.6)',
        }}>
          <button onClick={() => onSelect('W')}
            className="result-dropdown-btn result-dropdown-win" style={btnStyle}>
          ...
          <button onClick={() => onSelect('-')}
            className="result-dropdown-btn result-dropdown-pending" style={btnStyle}>
  ...
  return (
    <>
      <span ref={badgeRef} className={`result-badge ${badgeClass}`}
        ...
        {badgeText}
        {isAutoGraded && <i className="fa-solid fa-robot" style={{ fontSize: 9, opacity: 0.5 }}></i>}
